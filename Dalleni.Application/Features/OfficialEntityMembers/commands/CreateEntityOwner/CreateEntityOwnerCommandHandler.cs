...
namespace Dalleni.Application.Features.OfficialEntities.Members.CreateOwner
{
    ...
    public class CreateEntityOwnerCommandHandler : IRequestHandler<CreateEntityOwnerCommand, Response<bool>>
    {
        ...
        private readonly ILogger<CreateEntityOwnerCommandHandler> _logger;

        public CreateEntityOwnerCommandHandler(IResponseHandler responseHandler, IUnitOfWork unitOfWork, IEmailService emailService, IImageUploaderService imageUploaderService, ILogger<CreateEntityOwnerCommandHandler> logger)
        {
        ...
        public async Task<Response<bool>> Handle(CreateEntityOwnerCommand request, CancellationToken cancellationToken)
        {
            string? uploadedImageUrl = null;
            var dto = request.SignUpRequestDto;
            var fullName = $"{dto.FirstName} {dto.LastName}".Trim();

            var entity = await _unitOfWork.OfficialEntities.GetByIdAsync(request.entityId);
            if (entity is null)
            {
            ...
            try
            {
                ...
                var roleResult = await _unitOfWork.UserManager.AddToRoleAsync(user, "OWNER");
                ...
                // official entity owner creation logic
                var entityOwner = OfficialEntityMembership.Create(user.Id, request.entityId, EntityRole.Owner);
                await _unitOfWork.OfficialEntityMemberships.AddAsync(entityOwner, cancellationToken);
