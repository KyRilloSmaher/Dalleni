...
{
    internal class SendResetPasswordCodeCommandHnadler : IRequestHandler<SendResetPasswordCodeCommand, Response<bool>>
    {
        private readonly IResponseHandler _responseHandler;
        private readonly IUnitOfWork _unitOfWork;
        private readonly IEmailService _emailService;
        private readonly ILogger<SendResetPasswordCodeCommandHnadler> _logger;
        private readonly PasswordHasher<Dalleni.Domin.Models.ApplicationUser> _passwordHasher = new();

        public SendResetPasswordCodeCommandHnadler(IResponseHandler responseHandler, IUnitOfWork unitOfWork, IEmailService emailService, ILogger<SendResetPasswordCodeCommandHnadler> logger)
        {
        ...
        public async Task<Response<bool>> Handle(SendResetPasswordCodeCommand request, CancellationToken cancellationToken)
        {
            var dto = request.dto;
            ...
            {
                _logger.LogError("Failed to persist reset OTP for {Email}: {Errors}", dto.Email, string.Join(", ", updateResult.Errors.Select(e => e.Description)));
                return _responseHandler.BadRequest<bool>(SystemMessages.GENERATING_CODE_FAILED);
            }
