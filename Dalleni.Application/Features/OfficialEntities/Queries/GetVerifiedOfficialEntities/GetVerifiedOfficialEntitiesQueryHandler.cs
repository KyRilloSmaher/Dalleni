...
{
     internal sealed class GetAllOfficialEntitiesQueryHandler: IRequestHandler<GetVerifiedOfficialEntitiesQuery, Response<PaginatedResult<OfficialEntityDto>>>
    {
        ...
        public async Task< Response<PaginatedResult<OfficialEntityDto>>> Handle(GetVerifiedOfficialEntitiesQuery request,CancellationToken cancellationToken)
        {
            var PagedRequest = request.request;
            var query = await _unitOfWork.OfficialEntities.GetVerifiedAsync();
